...
- FR-01411: Sail sidecar must include only app-directory entries that are actually reachable in the state runtime, sourced from configured app-directory providers where possible: TraderX-hosted apps, frameable TradingView/Pricer demo apps ported from FDC3-Sail `main`, and FINOS conformance apps. Legacy local sample app records must not be advertised unless their backing ports are also started.
