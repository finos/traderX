...
### Fetch variables
set -a
: ${DATABASE_TCP_PORT:=8082}
: ${DATABASE_PG_PORT:=8083}
: ${DATABASE_WEB_PORT:=8084}
: ${DATABASE_DBUSER:=sa}
: ${DATABASE_DBPASS:=sa}
...
echo '---------------------------------------------------------------------------'
java -cp $DATABASE_H2JAR org.h2.tools.RunScript -url "jdbc:h2:$DATABASE_DATA_DIR/$DATABASE_DBNAME;TRACE_LEVEL_SYSTEM_OUT=3" -user $DATABASE_DBUSER -password $DATABASE_DBPASS -script initialSchema.sql
