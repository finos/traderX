...
# Grafana hardening defaults:
# - allow anonymous read-only dashboard access for demos
# - keep admin credentials configurable at runtime (generated by start scripts)
perl -0pi -e 's/GF_SECURITY_ADMIN_USER:\s*"admin"/GF_SECURITY_ADMIN_USER: "\${TRADERX_GRAFANA_ADMIN_USER:-admin}"/g' "${COMPOSE_FILE}"
perl -0pi -e 's/GF_SECURITY_ADMIN_PASSWORD:\s*"admin"/GF_SECURITY_ADMIN_PASSWORD: "\${TRADERX_GRAFANA_ADMIN_PASSWORD:-admin}"/g' "${COMPOSE_FILE}"
perl -0pi -e 's/GF_AUTH_ANONYMOUS_ENABLED:\s*"false"/GF_AUTH_ANONYMOUS_ENABLED: "\${TRADERX_GRAFANA_ANONYMOUS_ENABLED:-true}"/g' "${COMPOSE_FILE}"
...
perl -0pi -e 's#GF_SERVER_ROOT_URL:\s*".*"#GF_SERVER_ROOT_URL: "\${TRADERX_GRAFANA_ROOT_URL:-http://localhost:8080/grafana/}"#g' "${COMPOSE_FILE}"
