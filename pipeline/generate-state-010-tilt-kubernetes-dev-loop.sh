...
bash "${ROOT}/pipeline/render-state-010-tilt-kubernetes-dev-loop.sh"
bash "${ROOT}/pipeline/generate-state-architecture-doc.sh" "${STATE_ID}"

cat <<'EOF'
[summary] state=010-tilt-kubernetes-dev-loop
...
[summary] runtime-entrypoint=./scripts/start-state-010-tilt-kubernetes-dev-loop-generated.sh
EOF
