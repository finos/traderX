...
for required in "${UPSTREAM_BUILD_PLAN}" "${UPSTREAM_SPEC}"; do
  [[ -f "${required}" ]] || {
    echo "[fail] required state 009 artifact missing for state 012 render: ${required}"
    exit 1
  }
done
...
# State 012 Radius Platform Artifacts

Generated from:

- `specs/012-radius-kubernetes-platform/**`
- `generated/code/target-generated/kubernetes-runtime/build-plan.json`

State intent:

- preserve state 009 runtime behavior,
...

{
  ...
  echo
  echo "param application string = 'traderx-state-009'"
  echo
...
echo "[done] rendered state 012 radius artifacts into ${STATE_DIR}"
