...
TARGET="${GENERATED_ROOT}/code/components/position-service-specfirst"
...
jq -e '
  ...
  .component.id == "position-service" and
...
POSITION_SERVICE_PORT_ENV="$(manifest_env_by_prefix "POSITION_SERVICE_PORT")"
...
- Default port: \`${DEFAULT_PORT}\` via \`${POSITION_SERVICE_PORT_ENV}\`
