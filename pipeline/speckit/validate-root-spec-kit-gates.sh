...
for legacy_dir in system components conformance; do
  legacy_path="${TRADERSPEC_ROOT}/speckit/${legacy_dir}"
  if [[ -d "${legacy_path}" ]]; then
    extra_files="$(find "${legacy_path}" -maxdepth 1 -type f ! -name '_category_.json' ! -name 'README.md')"
    [[ -z "${extra_files}" ]] || fail "legacy ${legacy_dir} still contains non-pointer artifacts: ${extra_files}"
  fi
done

if [[ -d "${TRADERSPEC_ROOT}/speckit/contracts" ]]; then
  extra_contracts="$(find "${TRADERSPEC_ROOT}/speckit/contracts" -type f ! -name '_category_.json' ! -name 'README.md')"
