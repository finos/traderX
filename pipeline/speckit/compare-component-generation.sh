...
usage() {
  cat <<'EOF'
Usage:
  bash pipeline/speckit/compare-component-generation.sh <component-id> [legacy-ref] [--allow-differences]

Examples:
  bash pipeline/speckit/compare-component-generation.sh reference-data HEAD
  bash pipeline/speckit/compare-component-generation.sh trade-service origin/main
...
resolve_generator_script() {
  ...
  local candidate
  for candidate in "${GENERATOR_SCRIPT_CANDIDATES[@]}"; do
...
CURRENT_GENERATOR_SCRIPT="$(resolve_generator_script "${REPO_ROOT}" || true)"
if [[ -z "${CURRENT_GENERATOR_SCRIPT}" ]]; then
  echo "[fail] missing generator script in current workspace; tried:"
...
LEGACY_GENERATOR_SCRIPT="$(resolve_generator_script "${LEGACY_WORKTREE}" || true)"
if [[ -z "${LEGACY_GENERATOR_SCRIPT}" ]]; then
  echo "[fail] legacy ref does not contain generator script. Tried:"
  printf '  - %s\n' "${GENERATOR_SCRIPT_CANDIDATES[@]}"
  exit 1
fi
