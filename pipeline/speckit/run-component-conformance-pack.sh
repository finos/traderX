...
usage() {
  cat <<'EOF'
Usage:
  bash pipeline/speckit/run-component-conformance-pack.sh <component-id> [--execute-runtime-checks]
EOF
}
...
if [[ ! -f "${PACK_FILE}" ]]; then
  ...
  echo "[hint] run: bash pipeline/speckit/sync-conformance-packs.sh"
  exit 1
fi
...
while IFS= read -r verification_ref; do
  ...
  if (( EXECUTE_RUNTIME_CHECKS == 1 )) && [[ "${verification_ref}" == scripts/test-*.sh ]]; then
    "${verification_path}"
