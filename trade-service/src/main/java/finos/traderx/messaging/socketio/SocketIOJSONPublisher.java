 ...
 *
 */
public abstract class SocketIOJSONPublisher<T>
    implements Publisher<T>, InitializingBean {
  ...
  static {
    ...
    module.addSerializer(new TradeSideSerializer());
  ...
  public void publish(String topic, T message) throws PubSubException {
    if (!isConnected()) {
      throw new PubSubException(
          "Cannot send %s on topic %s - not connected".formatted(message,
                                                                   topic));
    }
    try {
      SocketIOEnvelope<T> envelope = new SocketIOEnvelope<T>(topic, message);
      String msgString = objectMapper.writerFor(SocketIOEnvelope.class)
