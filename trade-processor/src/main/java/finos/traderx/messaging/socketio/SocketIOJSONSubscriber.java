 ...
 */
public abstract class SocketIOJSONSubscriber<T>
    implements Subscriber<T>, InitializingBean {
  ...
  public SocketIOJSONSubscriber(Class<T> typeClass) {
    JavaType type = objectMapper.getTypeFactory().constructParametricType(
        SocketIOEnvelope.class, typeClass);
  ...
  public void setSocketAddress(String addr) { socketAddress = addr; }

  private String defaultTopic = "/default";
  public void setDefaultTopic(String topic) { defaultTopic = topic; }
  ...
  public void connect() throws PubSubException {
    ...
    try {
      ...
      throw new PubSubException("Cannot socket connection at " + socketAddress,
                                x);
    }
  }
      ...
      public void call(Object... args) {
        try {
          ...
          if (!objectType.getSimpleName().equals(json.get("type"))) {
            ...
            SocketIOEnvelope<T> envelope =
                (SocketIOEnvelope<T>)objectMapper.readValue(json.toString(),
                                                            envelopeType);
            log.info("Incoming Payload: " + envelope.getPayload());
            SocketIOJSONSubscriber.this.onMessage(envelope,
                                                  envelope.getPayload());
          }
