...
This will deploy all TraderX services to your local K8s cluster using Radius and automatically port-forward the application, which will be accessible at http://localhost:8080 once all services are running. Additionally, a [Radius Dashboard](https://docs.radapp.io/guides/tooling/dashboard/overview/) will be available at http://localhost:7007, which is the frontend experience for Radius and provides a graphical interface for visualizing the application graph for the TraderX application you have deployed. To stop the port-forwarding and terminate the application, press `Ctrl+C` in the terminal where the command is running.
