...
if [[ ! -f "${COMPOSE_FILE}" ]]; then
  ...
  echo "[hint] run: bash pipeline/generate-state.sh 005-messaging-nats-replacement"
  exit 1
fi
...
if docker compose -f "${COMPOSE_FILE}" --project-name "${COMPOSE_PROJECT_NAME}" ps --services | grep -q '^trade-feed$'; then
  echo "[error] state 005 runtime must not contain trade-feed service"
  exit 1
fi
...
echo "[done] state 005 messaging-nats runtime smoke tests passed"
