...
for required in \
  ...
  [[ -f "${required}" ]] || {
    echo "[error] missing generated state 010 artifact: ${required}"
    exit 1
  }
done
...
echo "[info] state 010 tilt assets ready at ${TILT_DIR}"
...
echo "[done] state 010 tilt-kubernetes dev loop ready (runtime inherited from state 009)"
