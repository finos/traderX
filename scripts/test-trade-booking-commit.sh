...
(cd "${TEST_ROOT}/processor" && bash ./gradlew test --no-daemon -Dbooking.schema="${TEST_ROOT}/database/initialSchema.sql")
...
(cd "${TEST_ROOT}/processor" && bash ./gradlew clean test --no-daemon -Dbooking.schema="${TEST_ROOT}/database/initialSchema.sql")
