...
echo "[info] state 012 radius artifacts ready at ${RADIUS_DIR}"
...
if command -v rad >/dev/null 2>&1; then
  ...
  echo "[info] radius CLI not found; runtime remains on state 009 Kubernetes path"
fi

echo "[done] state 012 radius-kubernetes platform started (runtime inherited from state 009)"
