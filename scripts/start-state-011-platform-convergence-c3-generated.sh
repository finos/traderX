...
STATE_ID="011-platform-convergence-c3"
...
for required in \
  ...
  [[ -f "${required}" ]] || {
    echo "[error] missing generated state 011 artifact: ${required}"
    exit 1
  }
done
...
"${REPO_ROOT}/scripts/start-state-009-kubernetes-runtime-generated.sh" "${start_args[@]}"
...
if (( DRY_RUN == 1 )); then
  ...
  echo "[done] dry run complete for state 011"
  exit 0
fi

echo "[info] state 011 convergence assets ready at ${TILT_DIR}"
...
echo "[done] state 011 platform convergence ready (runtime inherited from state 010/state 009)"
