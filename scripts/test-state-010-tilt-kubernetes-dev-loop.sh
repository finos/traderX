...
for required in \
  ...
  [[ -f "${required}" ]] || {
    echo "[error] missing expected state 010 artifact: ${required}"
    exit 1
  }
done
...
if grep -q "radius" "${TILTFILE}"; then
  echo "[error] Tiltfile should not include radius artifacts in state 010"
  exit 1
fi
...
echo "[done] state 010 tilt-kubernetes dev-loop smoke tests passed"
